...
// Heart Toggle + Like Counter
document.querySelectorAll('.heart-btn').forEach(function(btn){
    btn.addEventListener('click', function() {
        this.classList.toggle('text-red-500'); // Heart red toggle

        let likesValue = getNavValue('likes'); // Current likes value
        if(this.classList.contains('text-red-500')){
            document.getElementById('likes').innerText = likesValue + 1; // Increase counter
        } else {
            document.getElementById('likes').innerText = likesValue - 1; // Decrease counter
        }
    });
});
